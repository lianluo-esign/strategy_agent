...
def test_comprehensive_functionality():
    ...
    analyzer1 = EnhancedMarketAnalyzer(
        min_volume_threshold=Decimal("5.0"), analysis_window_minutes=180
    )

    result1 = analyzer1.analyze_market(
        snapshot=snapshot1, trade_data_list=[], symbol="BTCFDUSD", enhanced_mode=True
    )

    print(
        f"   ✅ 聚合结果: {len(result1.aggregated_bids)} bid levels, {len(result1.aggregated_asks)} ask levels"
    )
    print(
        f"   ✅ 深度统计: 压缩比 bids={result1.depth_statistics.get('bid_compression_ratio', 'N/A')}, asks={result1.depth_statistics.get('ask_compression_ratio', 'N/A')}"
    )
    print(
        f"   ✅ 体积保持率: {result1.depth_statistics.get('volume_preservation_rate', 'N/A')}%"
    )
    ...
    result2 = analyzer1.analyze_market(
        snapshot=snapshot2, trade_data_list=[], symbol="BTCFDUSD", enhanced_mode=True
    )

    print(
        f"   ✅ 稀疏数据处理: {len(result2.aggregated_bids)} bid levels, {len(result2.aggregated_asks)} ask levels"
    )
    ...
    performance_snapshot = DepthSnapshot(
        "BTCFDUSD",
        datetime.now(),
        performance_data[: len(bids1)],
        performance_data[len(bids1) :],
    )
    ...
    print(
        f"   ✅ 空订单簿处理: {len(empty_result.aggregated_bids)} bids, {len(empty_result.aggregated_asks)} asks"
    )
    ...
    print(
        f"   ✅ 单边订单簿处理: {len(single_result.aggregated_bids)} bids, {len(single_result.aggregated_asks)} asks"
    )
    ...
    print(
        f"   ✅ 大价格范围: {len(wide_result.aggregated_bids)} bids, {len(wide_result.aggregated_asks)} asks"
    )
    print(
        f"   ✅ 价格范围跨度: ${min(wide_result.aggregated_bids.keys() if wide_result.aggregated_bids else [0])} - ${max(wide_result.aggregated_asks.keys() if wide_result.aggregated_asks else [0])}"
    )
