...
def aggregate_depth_by_one_dollar(bids: List[DepthLevel], asks: List[DepthLevel]) -> Tuple[Dict[Decimal, Decimal], Dict[Decimal, Decimal]]:
    """
    ...
    """
    aggregated_bids = defaultdict(Decimal)
    aggregated_asks = defaultdict(Decimal)

    logger.debug(f"Aggregating {len(bids)} bid levels and {len(asks)} ask levels")

    # Aggregate bids
    for bid_level in bids:
        if not isinstance(bid_level.price, Decimal) or not isinstance(bid_level.quantity, Decimal):
            logger.warning(f"Invalid bid level format: {bid_level}")
            continue
    ...
    for ask_level in asks:
        if not isinstance(ask_level.price, Decimal) or not isinstance(ask_level.quantity, Decimal):
            logger.warning(f"Invalid ask level format: {ask_level}")
            continue
...
def calculate_depth_statistics(aggregated_bids: Dict[Decimal, Decimal], aggregated_asks: Dict[Decimal, Decimal]) -> Dict[str, Decimal]:
    ...
    """
    total_bid_volume = sum(aggregated_bids.values()) if aggregated_bids else Decimal('0')
    total_ask_volume = sum(aggregated_asks.values()) if aggregated_asks else Decimal('0')
    ...
    bid_ask_ratio = total_bid_volume / (total_ask_volume + Decimal('0.01'))  # Avoid division by zero
...
def validate_aggregation_quality(
    ...
    aggregated_bid_volume = sum(aggregated_bids.values()) if aggregated_bids else Decimal('0')
    aggregated_ask_volume = sum(aggregated_asks.values()) if aggregated_asks else Decimal('0')
    total_aggregated_volume = aggregated_bid_volume + aggregated_ask_volume

    # Calculate quality metrics
    volume_preservation = (total_aggregated_volume / (total_original_volume + Decimal('0.01'))) * Decimal('100')
    ...
    if max_original_volume > Decimal('0'):
        aggregation_efficiency = (total_original_volume / (original_price_levels * max_original_volume + Decimal('0.01'))) * Decimal('100')
    else:
        aggregation_efficiency = Decimal('0')

    quality_metrics = {
        'volume_preservation': volume_preservation,
        'price_levels_reduction': price_levels_reduction,
