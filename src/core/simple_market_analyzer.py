...
from .models import DepthSnapshot, DepthLevel
...
class SimpleMarketAnalyzer:
    ...
    def __init__(
        ...
        self.price_aggregator = PriceAggregator(
            precision=price_aggregation_precision,
            enabled=True,
            max_price_levels=5000
        )
        logger.info(f"Initialized price aggregator with precision=${price_aggregation_precision}")
    ...
    def analyze_market(self, symbol: str = "BTCFDUSD") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Retrieved depth snapshot: {snapshot.symbol} from {snapshot.timestamp}")
            ...
            if self.deepseek_analyzer:
                try:
                    ...
                    deepseek_analysis = self.deepseek_analyzer.analyze_order_book_with_llm(
    ...
    def _log_deepseek_analysis(self, analysis_result: Dict[str, Any]) -> None:
        ...
        if analysis_result.get("status") != "success":
            logger.info(f"❌ DeepSeek分析失败: {analysis_result.get('error', '未知错误')}")
            return
        ...
        if structured_analysis:
            ...
            if "市场平衡" in structured_analysis:
                ...
                if balance.get('分析'):
                    logger.info(f"   分析: {balance['分析'][:100]}{'...' if len(balance['分析']) > 100 else ''}")

            # 打印关键价位
            if "关键价位" in structured_analysis:
                logger.info("📍 关键价位:")
                for i, key_level in enumerate(structured_analysis["关键价位"][:5], 1):  # 限制前5个
                    logger.info(
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "deepseek_analysis": {
                ...
                "model": self.deepseek_analyzer.model if self.deepseek_analyzer else None,
            },
            "visualization": {
