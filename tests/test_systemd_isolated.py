...
class TestSystemdConfigIsolated:
    """Isolated systemd service configuration tests."""
    ...
    def test_all_systemd_files_exist(self):
        """Test that all expected systemd files exist."""
        ...
        for filename in expected_files:
            ...
            assert file_path.is_file(), f"Should be a file: {filename}"

    def test_logrotate_directory_structure(self):
        """Test logrotate directory structure."""
        logrotate_dir = Path(__file__).parent.parent / "systemd" / "logrotate.d"
        if logrotate_dir.exists():
            assert logrotate_dir.is_dir(), "Should be a directory"

            config_file = logrotate_dir / "strategy-agent-data-collector"
            if config_file.exists():
                assert config_file.is_file(), "Should be a file"

    def test_install_script_content(self):
        """Test install script has required content."""
        install_path = Path(__file__).parent.parent / "systemd" / "install.sh"
        ...
        for command in required_commands:
            assert command in content, f"Missing command: {command}"

        # Check for error handling
        assert "set -e" in content, "Should exit on error"

        # Check for user check
        assert "EUID -ne 0" in content, "Should check for root privileges"
