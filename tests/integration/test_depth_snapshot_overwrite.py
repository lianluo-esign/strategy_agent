        ...
        with patch("src.core.redis_client.redis.Redis", return_value=mock_redis):
            ...
            for i in range(5):
                snapshot = DepthSnapshot(
                    ...
                    bids=[
                        DepthLevel(
                            price=Decimal(f"5000{i}.00"), quantity=Decimal(f"1.{i}")
                        ),
                        DepthLevel(
                            price=Decimal(f"5000{i - 1 if i > 0 else 0}.00"),
                            quantity=Decimal(f"2.{i}"),
                        ),
                    ],
                    asks=[
                        DepthLevel(
                            price=Decimal(f"5000{i}.01"), quantity=Decimal(f"1.{i}")
                        ),
                        DepthLevel(
                            price=Decimal(f"5000{i + 1}.01"), quantity=Decimal(f"2.{i}")
                        ),
                    ],
                )
