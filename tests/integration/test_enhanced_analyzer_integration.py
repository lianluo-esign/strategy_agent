...
class TestEnhancedAnalyzerIntegration:
    ...
    def sample_market_data(self) -> tuple[DepthSnapshot, list[MinuteTradeData]]:
        ...
        bids = [
            DepthLevel(
                price=Decimal(f"{100 - i * 0.05}"), quantity=Decimal(f"{10 + i * 1.5}")
            )
        ...
        asks = [
            DepthLevel(
                price=Decimal(f"{100 + i * 0.05}"), quantity=Decimal(f"{8 + i * 1.2}")
            )
        ...
        for minute_offset in range(180):  # 3 hours = 180 minutes
            ...
            base_activity = 10 + (
                minute_offset % 30
            )  # Activity varies every 30 minutes
    ...
    @pytest.mark.asyncio
    async def test_redis_integration_workflow(
        self, redis_store, analyzer, sample_market_data
    ):
    ...
    def test_performance_under_load(self, analyzer):
        ...
        large_bids = [
            DepthLevel(price=Decimal(f"{100 - i * 0.001}"), quantity=Decimal("10"))
        ...
        total_aggregated_levels = len(result.aggregated_bids) + len(
            result.aggregated_asks
        )
...
class TestEnhancedAnalyzerErrorHandling:
    ...
    def test_analysis_with_corrupted_depth_data(self, analyzer):
        ...
        corrupted_bids = [
            DepthLevel(
                price=Decimal("100"), quantity=Decimal("-5")
            ),  # Negative quantity
        ...
        corrupted_asks = [
            DepthLevel(
                price=Decimal("100"), quantity=Decimal("inf")
            ),  # Infinite quantity (will error)
