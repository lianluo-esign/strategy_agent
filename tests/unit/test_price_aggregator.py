...
class TestAggregateDepthByOneDollar:
    ...
    def test_aggregate_basic_functionality(self, sample_bids, sample_asks):
        ...
        assert aggregated_bids[Decimal("99")] == Decimal(
            "42.0"
        )  # 99.50+99.20+99.80+99.10
        ...
        assert aggregated_asks[Decimal("100")] == Decimal(
            "31.0"
        )  # 100.10+100.20+100.50+100.90
...
class TestIdentifyLiquidityClusters:
    ...
    def test_identify_basic_liquidity_clusters(self):
        ...
        clusters = identify_liquidity_clusters(
            depth_data, min_cluster_volume=Decimal("10.0")
        )
    ...
    def test_identify_clusters_with_low_volume_threshold(self):
        ...
        clusters = identify_liquidity_clusters(
            depth_data, min_cluster_volume=Decimal("5.0")
        )
...
class TestValidateAggregationQuality:
    ...
    def test_validate_high_quality_aggregation(self):
        ...
        quality = validate_aggregation_quality(
            original_levels, aggregated_data, original_volume
        )
    ...
    def test_quality_score_calculation(self):
        ...
        quality1 = validate_aggregation_quality(
            100, {Decimal("99"): Decimal("100.0")}, Decimal("1000.0")
        )

        # Test case with poor compression but good preservation
        quality2 = validate_aggregation_quality(
            10, {Decimal("99"): Decimal("900.0")}, Decimal("1000.0")
        )
...
class TestIntegrationScenarios:
    ...
    def test_complete_aggregation_pipeline(self):
        ...
        raw_asks = [
            DepthLevel(
                price=Decimal(f"{100 + i * 0.1}"), quantity=Decimal(f"{8 + i * 0.5}")
            )
            for i in range(20)
        ]
